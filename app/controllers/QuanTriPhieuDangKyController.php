...
class QuanTriPhieuDangKyController extends BaseController {
    ...
    public function trangthaiphieu() {
        $id_phieu_dang_ki = Input::get('id_phieu_dang_ky');
        $trang_thai = Input::get('trang_thai');
        DB::table('phieu_dang_kies')
                ->where('id', $id_phieu_dang_ki)
                ->update(array('trang_thai' => $trang_thai));
    }
