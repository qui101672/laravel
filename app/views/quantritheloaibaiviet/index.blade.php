...
@if ($the_loai_bai_viets->count())
		...
		<tbody>
			@foreach ($the_loai_bai_viets as $the_loai_bai_viet)
				<tr>
                    ...
                    <td>{{ link_to_route('the_loai_bai_viet.edit', 'Edit', array($the_loai_bai_viet->id), array('class' => 'btn btn-info')) }}</td>
                    <td>
                        {{ Form::open(array('method' => 'DELETE', 'route' => array('the_loai_bai_viet.destroy', $the_loai_bai_viet->id))) }}
	...
	{{ link_to_route('the_loai_bai_viet.create', 'Tạo Thể Loại Bài Viết',array() ,array('class' => 'btn btn-info'))}}
